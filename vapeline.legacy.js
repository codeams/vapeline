 ...
 *
 */
let search = (list, attrs, value, options) => {
  let { caseSensitive: sensitive = false } = options
  let { partialMatches: permissive = true } = options

  return list.filter(elem => {
    let satisfy = false
    for (let index in attrs) {
      let compareTo = elem[attrs[index]]
...
module.exports = { vape, pipe, search }
